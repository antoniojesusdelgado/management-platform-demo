        ...
        <Dialog.Portal>
          ...
          <Dialog.Content className="dialog-content workspace-command-dialog" aria-describedby="workspace-command-description">
            ...
            <div className="workspace-command-results" role="region" aria-label={tab === "search" ? "Resultados de búsqueda" : "Elementos de la bandeja"} aria-busy={loading} onKeyDown={handleListKeyDown} tabIndex={tab === "inbox" ? 0 : -1}>
              ...
              })}</section>) : inbox.map((item, index) => <button type="button" aria-current={index === activeIndex ? "true" : undefined} className={`workspace-command-item${index === activeIndex ? " active" : ""}`} key={`${item.kind}-${item.id}`} onMouseEnter={() => setActiveIndex(index)} onClick={() => openItem(item)}><span className={`workspace-command-priority priority-${item.priority}`} aria-hidden="true" /><span><strong>{item.title}</strong><small>{item.description}{item.dueAt ? ` · ${item.dueAt}` : ""}</small></span><span className={`status priority-label-${item.priority}`}>{priorityLabels[item.priority]}</span><IconChevronRight size={18} aria-hidden="true" /></button>)}
            </div>
          </Dialog.Content>
        </Dialog.Portal>
      </Dialog.Root>
