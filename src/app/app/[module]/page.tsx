...
export default async function AppModulePage({
  ...
  if (module === "nominas") {
    ...
    payrollRuns = (runsResult.data ?? []).map((run) => ({ id: run.id, periodStart: run.period_start, periodEnd: run.period_end, peopleCount: run.people_count, grossTotalCents: run.gross_total_cents, deductionTotalCents: run.deduction_total_cents, netTotalCents: run.net_total_cents, currency: run.currency as PayrollCurrency, notes: run.notes, status: run.status, createdBy: (run.creator as unknown as { display_name: string }).display_name, createdAt: run.created_at, updatedAt: run.updated_at }));
