...
describe("guest demo", () => {
  ...
  test("restores version 11 sessions once with the balanced V6 scenario", () => {
    ...
    expect(migrated?.scenarioVersion).toBe(7);
    expect(migrated?.payrollRuns.length).toBeGreaterThan(0);
    expect(migrated?.changelogEntries.at(-1)?.version).toBe("1.8.2");
  });
  ...
  test("adds v1.6.0 once to version 19 sessions", () => {
    const legacyEntries = initialGuestDemoState.changelogEntries.filter(
      (entry) => entry.version !== "1.6.0",
    );
    const migrated = parseGuestDemoState({
      ...initialGuestDemoState,
      version: 19,
      changelogEntries: legacyEntries,
      changelogEvents: initialGuestDemoState.changelogEvents.filter(
        (event) => !initialGuestDemoState.changelogEntries.some(
          (entry) => entry.version === "1.6.0" && entry.id === event.entryId,
        ),
      ),
    });
